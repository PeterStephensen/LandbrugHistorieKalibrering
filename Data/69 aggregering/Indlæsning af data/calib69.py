...
EKtax_privat=df.tCorp['Pt'].loc['Privat']*df.rTaxDep_rest/(r.loc['Privat']+df.rTaxDep_rest)
# Giv dem samme indeksnavn (tid)
EKtax_lan.index.name = t
EKtax_fre.index.name = t
EKtax_rest.index.name = t
EKtax_privat.index.name = t
...
EKtax_privat.index = pd.MultiIndex.from_product([['Privat'],  EKtax_privat.index], names=[i, t])
# Saml til én variabel
EKtax = pd.concat([EKtax_lan, EKtax_fre, EKtax_rest, EKtax_privat]).sort_index()
...
P_K_final = P_K.reset_index(name='Pt')
P_K_final.to_csv(DATA69_DIR / 'P_K.csv', index=False)


# Bestem først samlet dansk produktion ved at summe over tilgang i M_D
M_D_tot = df.M_D_loebende.loc[df.M_D_loebende.index.get_level_values("TILGANG2") != "Privat"].groupby(['ANVENDELSE', 'TID']).sum()
...
tau_MD_final.to_csv(DATA69_DIR / 'tau_MD.csv', index=False)
...
P_MxM_tot = P_MxM.loc[P_MxM.index.get_level_values("TILGANG2") != "Privat"].groupby(['ANVENDELSE', 'TID']).sum()
...
P_J_final.to_csv(DATA69_DIR / 'P_J.csv', index=False)
