...
##################################################################################################
df_materialer_MD_l = (
    df_materialer_MD_lob.loc[df_materialer_MD_lob.index.get_level_values("TILGANG2") != "Privat"]
...
df_materialer_MD_f = (
    df_materialer_MD_for.loc[df_materialer_MD_for.index.get_level_values("TILGANG2") != "Privat"]
                        .groupby(['ANVENDELSE', 'TID'])['INDHOLD']
                        .sum()
)
...
##################################################################################################
df_materialer_MF_l = (
    df_materialer_MF_lob.loc[df_materialer_MF_lob.index.get_level_values("TILGANG2") != "Privat"]
                        .groupby(['ANVENDELSE', 'TID'])['INDHOLD']
                        .sum()
)
...
df_materialer_MF_f = (
    df_materialer_MF_for.loc[df_materialer_MF_for.index.get_level_values("TILGANG2") != "Privat"]
                        .groupby(['ANVENDELSE', 'TID'])['INDHOLD']
                        .sum()
)
...
loebende_Mtot = (
    df_materialer_aggregat_loebende
    .loc[df_materialer_aggregat_loebende.index.get_level_values("TILGANG2") != "Privat"]
    .groupby(['ANVENDELSE', 'TID'])['INDHOLD']
    .sum()
)
...
for_Mtot = (df_Xt_Mjit.loc[df_Xt_Mjit["TILGANG2"] != "Privat"].set_index(['TILGANG2', 'ANVENDELSE', 'TID'])['Xt'] * df_Pt_prev).groupby(['ANVENDELSE', 'TID']).sum()
